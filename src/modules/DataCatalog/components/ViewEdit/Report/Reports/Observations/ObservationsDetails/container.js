...
function mapStateToProps(state) {
  const reportData = get(state, 'dataCatalog.reportDetails.data', {});
  let observationsByType = get(reportData, 'OBSERVATIONS_BY_TYPE');
  ...
  if (observationsByType) {
    observationsByType = chart.normalizeDataframe(observationsByType);
  }

  if (ageAtFirstOccurrence) {
    ageAtFirstOccurrence = chart.prepareData(ageAtFirstOccurrence, chart.chartTypes.BOXPLOT);
  }
